...
import { CheckCircle, Circle, Clock, BookOpen, Coffee, Target, Zap, Trophy, Calendar, Brain } from "lucide-react";
...
export const StudyPlanDisplay = memo(({ sessions, onToggleTask, analytics }: StudyPlanDisplayProps) => {
  ...
  const getTaskIcon = (type: string) => {
    switch (type) {
      ...
      case "review": return <Brain className="h-4 w-4 text-blue-600" />;
      case "exam": return <Trophy className="h-4 w-4 text-blue-700" />;
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      ...
      case "medium": return "bg-orange-100 text-orange-800 dark:bg-orange-900/40 dark:text-orange-300";
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center space-y-4">
        <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-100 to-blue-200 dark:from-blue-900/30 dark:to-blue-800/30 rounded-full">
          <Trophy className="h-5 w-5 text-blue-600 dark:text-blue-400" />
          <span className="text-sm font-medium text-blue-700 dark:text-blue-300">Your Study Plan is Ready!</span>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <Card className="p-3 sm:p-4 text-center dark:bg-slate-900 border-2 border-blue-200 dark:border-blue-800 hover:shadow-lg transition-shadow">
          <Clock className="h-5 w-5 sm:h-6 sm:w-6 text-blue-500 mx-auto mb-2" />
          <div className="text-xl sm:text-2xl font-bold text-blue-600 dark:text-blue-400">{Math.round(analytics.totalHours)}h</div>
        ...
        <Card className="p-3 sm:p-4 text-center dark:bg-slate-900 border-2 border-blue-200 dark:border-blue-800 hover:shadow-lg transition-shadow">
          ...
          <div className="text-xl sm:text-2xl font-bold text-blue-600 dark:text-blue-400">{analytics.completedTasks}</div>
          <div className="text-xs text-slate-600 dark:text-slate-400">Tasks Completed</div>
        </Card>
        
        <Card className="p-3 sm:p-4 text-center dark:bg-slate-900 border-2 border-blue-200 dark:border-blue-800 hover:shadow-lg transition-shadow">
          <Zap className="h-5 w-5 sm:h-6 sm:w-6 text-blue-500 mx-auto mb-2" />
          <div className="text-xl sm:text-2xl font-bold text-blue-600 dark:text-blue-400">{Math.round(analytics.efficiency)}%</div>
          <div className="text-xs text-slate-600 dark:text-slate-400">Efficiency Rate</div>
        </Card>
        
        <Card className="p-3 sm:p-4 text-center dark:bg-slate-900 border-2 border-blue-200 dark:border-blue-800 hover:shadow-lg transition-shadow">
          <Trophy className="h-5 w-5 sm:h-6 sm:w-6 text-blue-500 mx-auto mb-2" />
          <div className="text-xl sm:text-2xl font-bold text-blue-600 dark:text-blue-400">{analytics.streak}</div>
      ...
      {/* Study Tips */}
      <StudyTips analytics={analytics} />
    </div>
  );
});

StudyPlanDisplay.displayName = "StudyPlanDisplay";
