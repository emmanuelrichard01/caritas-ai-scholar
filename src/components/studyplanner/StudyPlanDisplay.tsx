...
export const StudyPlanDisplay = ({ sessions, onToggleTask, analytics }: StudyPlanDisplayProps) => {
  ...
  const getTaskIcon = (type: string) => {
    switch (type) {
      ...
      case "practice": return <Target className="h-4 w-4 text-red-500" />;
      case "review": return <Brain className="h-4 w-4 text-red-600" />;
      case "exam": return <Trophy className="h-4 w-4 text-red-700" />;
      default: return <BookOpen className="h-4 w-4 text-red-500" />;
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center space-y-4">
        <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-red-100 to-red-200 dark:from-red-900/30 dark:to-red-800/30 rounded-full">
          <Trophy className="h-5 w-5 text-red-600 dark:text-red-400" />
          <span className="text-sm font-medium text-red-700 dark:text-red-300">Your Study Plan is Ready!</span>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-4 text-center dark:bg-slate-900 border-2 border-red-200 dark:border-red-800">
          <Clock className="h-6 w-6 text-red-500 mx-auto mb-2" />
          <div className="text-2xl font-bold text-red-600 dark:text-red-400">{Math.round(analytics.totalHours)}h</div>
        ...
        <Card className="p-4 text-center dark:bg-slate-900 border-2 border-red-200 dark:border-red-800">
          ...
          <div className="text-2xl font-bold text-red-600 dark:text-red-400">{analytics.completedTasks}</div>
        ...
        <Card className="p-4 text-center dark:bg-slate-900 border-2 border-red-200 dark:border-red-800">
          ...
          <div className="text-2xl font-bold text-red-600 dark:text-red-400">{Math.round(analytics.efficiency)}%</div>
        ...
        <Card className="p-4 text-center dark:bg-slate-900 border-2 border-red-200 dark:border-red-800">
          ...
          <div className="text-2xl font-bold text-red-600 dark:text-red-400">{analytics.streak}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {sessions.map((session, index) => (
          <Card key={session.id} className="overflow-hidden dark:bg-slate-900 hover:shadow-lg transition-shadow duration-300">
            {/* Session Header */}
            <div className="bg-gradient-to-r from-slate-50 to-red-50 dark:from-slate-800 dark:to-red-900/20 p-6">
      ...
      {/* Study Tips */}
      <Card className="p-6 dark:bg-slate-900 bg-gradient-to-r from-red-50 to-red-100 dark:from-red-900/20 dark:to-red-800/20 border-2 border-red-200 dark:border-red-800">
