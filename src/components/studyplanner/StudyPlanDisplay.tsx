...
export const StudyPlanDisplay = ({ sessions, onToggleTask, analytics }: StudyPlanDisplayProps) => {
  ...
  const getTaskIcon = (type: string) => {
    switch (type) {
      ...
      case "practice": return <Target className="h-4 w-4 text-caritas" />;
      case "review": return <Brain className="h-4 w-4 text-caritas-light" />;
      case "exam": return <Trophy className="h-4 w-4 text-yellow-500" />;
      default: return <BookOpen className="h-4 w-4 text-caritas-dark" />;
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center space-y-4">
        <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-caritas/10 to-caritas-light/10 dark:from-caritas/20 dark:to-caritas-light/20 rounded-full">
          <Trophy className="h-5 w-5 text-caritas" />
          <span className="text-sm font-medium text-caritas-dark dark:text-caritas-light">Your Study Plan is Ready!</span>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-4 text-center dark:bg-slate-900 border-2 border-caritas/20">
          <Clock className="h-6 w-6 text-caritas mx-auto mb-2" />
          <div className="text-2xl font-bold text-caritas">{Math.round(analytics.totalHours)}h</div>
          <div className="text-xs text-slate-600 dark:text-slate-400">Total Study Time</div>
        </Card>
        
        <Card className="p-4 text-center dark:bg-slate-900 border-2 border-caritas-light/20">
          <Target className="h-6 w-6 text-caritas-light mx-auto mb-2" />
          <div className="text-2xl font-bold text-caritas-light">{analytics.completedTasks}</div>
          <div className="text-xs text-slate-600 dark:text-slate-400">Tasks Completed</div>
        </Card>
        
        <Card className="p-4 text-center dark:bg-slate-900 border-2 border-caritas-dark/20">
          <Zap className="h-6 w-6 text-caritas-dark mx-auto mb-2" />
          <div className="text-2xl font-bold text-caritas-dark">{Math.round(analytics.efficiency)}%</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {sessions.map((session, index) => (
          <Card key={session.id} className="overflow-hidden dark:bg-slate-900 hover:shadow-lg transition-shadow duration-300">
            {/* Session Header */}
            <div className="bg-gradient-to-r from-caritas/5 to-caritas-light/5 dark:from-caritas/10 dark:to-caritas-light/10 p-6">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <Calendar className="h-4 w-4 text-caritas" />
                    <h3 className="text-lg font-semibold dark:text-white">
                      Day {index + 1} - {formatSessionDate(session.date)}
                    </h3>
                  </div>
                ...
                <div className="text-right">
                  <div className="text-3xl font-bold text-caritas">
      ...
      {/* Study Tips */}
      <Card className="p-6 dark:bg-slate-900 bg-gradient-to-r from-caritas/5 to-caritas-light/5 dark:from-caritas/10 dark:to-caritas-light/10 border-2 border-caritas/20">
        <div className="flex items-center gap-2 mb-4">
          <Brain className="h-6 w-6 text-caritas" />
          <h3 className="text-xl font-semibold dark:text-white">Study Success Tips</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-3">
            <div className="flex items-start gap-3">
              <CheckCircle className="h-5 w-5 text-caritas flex-shrink-0 mt-0.5" />
              <div>
